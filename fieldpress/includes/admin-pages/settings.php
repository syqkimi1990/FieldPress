...
if ( isset( $_POST['_wpnonce'] ) && current_user_can( 'manage_options' ) ) {
	if ( wp_verify_nonce( $_REQUEST['_wpnonce'], 'update-fieldpress-options' ) ) {
		...
		do_action( 'fieldpress_update_settings', $tab, $_POST );
	}
...
<div class='wrap mp-wrap nofieldsub cp-wrap' id="settings-wrap">
	...
	/* if ( current_user_can( 'manage_options' ) || current_user_can( 'fieldpress_settings_groups_page_cap' ) ) {
	...
	if ( current_user_can( 'fieldpress_settings_shortcode_page_cap' ) || current_user_can( 'manage_options' ) ) {
	...
	$menus = apply_filters( 'fieldpress_settings_new_menus', $menus );
	?>
	...
	<div id="undefined-sticky-wrapper" class="sticky-wrapper">
		...
		<ul class="mp-tabs" style="">
			...
			?>
			<li class="mp-tab">
				<a class="mp-tab-link" href="<?php echo admin_url( 'admin.php?page=fields&quick_setup' ); ?>"><?php _e( 'View Setup Guide', 'cp' ); ?></a>
			</li>
		</ul>
	</div>
	...
	<div class='mp-settings'>
		...
		switch ( $tab ) {
			...
			case 'cp-leafletmapmaker':
				//if ( current_user_can( 'manage_options' ) ) {
				$this->show_settings_leafletmapmaker();
				//}
				break;

			default:
				do_action( 'fieldpress_settings_menu_' . $tab );
				break;
		}
		?>
	</div>
